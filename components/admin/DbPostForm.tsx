...
export default function DbPostForm({ post }: Props) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const payload = { title, slug, description, date, author, category, coverImage: coverImage || null, content, published }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="md:col-span-2">
          ...
          <input value={slug} onChange={(e) => { setSlugTouched(true); setSlug(e.target.value) }} required
        ...
        <div className="flex items-center gap-3 pt-6">
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-sm text-[#F0F4FF]">Published</span>
          </label>
        </div>

        <div className="md:col-span-2">
          <label className="block text-xs font-medium text-[#6B7FA3] mb-2">Featured Image</label>
          <input ref={fileInputRef} type="file" accept="image/jpeg,image/png,image/webp,image/gif,image/svg+xml" className="hidden"
          ...
          {coverImage ? (
            <div className="relative rounded-xl overflow-hidden border border-white/10 h-48 group">
              ...
              <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                ...
                <button type="button" onClick={() => { setCoverImage(''); if (fileInputRef.current) fileInputRef.current.value = '' }}
              ...
              {uploading && <div className="absolute inset-0 bg-black/70 flex items-center justify-center"><span className="text-[#00D4FF] text-sm">Uploading...</span></div>}
            </div>
          ) : (
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-1 p-1 rounded-full bg-white/5 border border-white/10">
            <button type="button" onClick={() => setTab('edit')}
              className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${tab === 'edit' ? 'bg-[#00D4FF] text-black' : 'text-[#6B7FA3]'}`}>
              <Pencil size={12} /> Edit
            </button>
            <button type="button" onClick={() => setTab('preview')}
              className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${tab === 'preview' ? 'bg-[#00D4FF] text-black' : 'text-[#6B7FA3]'}`}>
              <Eye size={12} /> Preview
            </button>
          </div>
        </div>
        {tab === 'edit' ? (
          ...
          <div className="mdx-content px-4 py-3 rounded-lg bg-white/5 border border-white/10 min-h-[400px] max-h-[600px] overflow-y-auto"
